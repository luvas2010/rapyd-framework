...
class datagrid_library extends dataset_library {
	...
	public function set_column() {
		...
		if (!isset($column['source'])) $column['source'] = $this->source;
		//detect if is a checkbox column
		if (isset($column['checkbox'])) $this->checkbox_form = true;
...
class datagrid_column extends rpd_component_library {
	...
	public function set_row($data_row) {
		...
		if ($this->url) {
			if (!isset($this->attributes['style'])) $this->attributes['style'] = 'width: 70px; text-align:center; padding-right:5px';

...
class fSQLEnvironment
{
	...
	function define_db($name, $path)
	{
		...
		if(is_dir($path) && is_readable($path) && is_writeable($path.$name)) {
			...
			if(!isset($matches[6])) {
				...
				for($c = 0; $c < count($Columns[0]); $c++) {
					...
					else
					{
						...
						if(strpos($options, "not null"))
							$null = 0;
						else
							$null = 1;

						if(strpos($options, "AUTO_INCREMENT"))
							$auto = 1;
						else
							$auto = 0;
	...
	*/
	...
	function _query_select($query)
	{
		...
		foreach($selects as $select) {
			...
			$distinct = false;// !strncasecmp($matches[1], "DISTINCT", 8);
			$has_random = false;// @$matches[2] != " ";
	...
	function fetch_array($id, $type = 1)
	{
		...
		foreach($this->Columns[$id] as $column) {
			...
			if(preg_match("/\A(?:([A-Z][A-Z0-9\_]*)\.)?([A-Z][A-Z0-9\_]*)(?:\s+(?:AS\s+)?([A-Z][A-Z0-9\_]*))?\Z/is",$column,$matches)) {
				...
				$newentry = (isset($newentry)) ? $this->_load_functions($id, $column, $entry, $newentry) : $this->_load_functions($id, $column, $entry, array());
			}
			else {
				$load = $entry[$column];
				$load = strtr($load, array("\\\"" => "\"", "\\\\\"" => "\\\""));
